...
const config = {
  ...
  logo: () => {
    ...
    return (
      <>
        ...
        {/* {route === '/' ? null : ( */}
        <span className='mx-2 select-none font-bold' title='MyVacation Docs' style={{ whiteSpace: 'nowrap' }}>
          MyVacation API Docs
        </span>
        {/* )} */}
      </>
  ...
  project: {
    link: 'https://github.com/wahid-ari/next-supabase-vacation-docs',
  },
  // chat: {
  ...
  useNextSeoProps() {
    ...
    if (route !== '/') {
      return {
        titleTemplate: '%s – MyVacation API Docs',
      };
    }
  },
  head: function useHead() {
    ...
    const { route } = useRouter();
    const socialCard =
      route === '/' || !title ? `${process.env.BASE_URL}/og.png` : `${process.env.BASE_URL}/api/og?title=${title}`;
    ...
    return (
      <>
        ...
        <meta name='og:title' content={`${title} - MyVacation API Docs`} />
        <meta name='description' content={`${title} - Discover MyVacation API Docs`} />
        <meta name='og:description' content={`${title} - Discover MyVacation API Docs`} />
        ...
        <meta name='apple-mobile-web-app-title' content='MyVacation' />
    ...
    text: () => {
      return (
        <div>
          ...
          <p className='mt-4 text-center text-[13px] md:text-left'>© {new Date().getFullYear()} MyVacation.</p>
        </div>
      );
    },
  },
